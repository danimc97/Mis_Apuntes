		...
		for (int i=0; i<lista_ataque_personaje2.size();i++) {
			...
			if (lista_ataque_personaje2.get(i) instanceof Punetazo) {
				...
				if(probabilidad<75) {
					...
					System.out.println(this.getNombre()+" ha dado un pu�etazo a "+Personaje1.getInstancia().getNombre());
				}
				else {
					System.out.println(this.getNombre()+" ha dado un pu�etazo a "+Personaje1.getInstancia().getNombre());
					System.out.println(Personaje1.getInstancia().getNombre()+" se ha defendido del pu�etazo");
				}
			}
